...
### Non-boolean settings and the native mirror pattern (`src/shared/chart-shortcuts/`)

`native-settings.ts`'s original `settings.getAll`/`settings.setOne` native-messaging pair is boolean-and-`PageKey`-shaped end to end (TS types down to the Swift `UserDefaults` handler's `as? Bool` guard), so it can't carry arbitrary settings. The chart-shortcuts keybindings feature established the pattern for a second, parallel setting instead of widening that one: a new `keybindings.get`/`keybindings.setAll` native message pair carrying a single JSON-encoded string, stored in `UserDefaults` under a key **outside** the `pages.` prefix (`keybindings.chartShortcuts`) — `handleGetAll`'s scan only looks at `pages.`-prefixed keys and does an `as? Bool` cast, so anything stored inside that prefix would be silently dropped. Follow this same shape (a dedicated message pair, a distinct non-`pages.`-prefixed UserDefaults key, JSON-string payload) for any future non-boolean setting rather than trying to widen the existing boolean channel.

Read/write order in `shared/chart-shortcuts/settings.ts` (the reusable half of this pattern): reads ask the background for the native copy first and self-heal `storage.local` from it on a hit; writes always hit `storage.local` first (the source of truth, works with no native host on Chrome/Firefox) and then best-effort mirror to native, swallowing failure.

`subscribeToChartShortcutBindings` gives an already-running context (a content script on an open chart page) a live update when a change is made in another context (the popup), without needing a page reload — useful for any future setting a long-lived content script needs to react to immediately rather than only reading once at startup. It originally wrapped `browser.storage.onChanged` directly, but that was found (2026-08-22) to never fire in a content script's context for a write made from the popup on this Safari build — a cross-context propagation gap, confirmed empirically with temporary logging (the write itself was correct; a page reload always picked it up, but the live listener never received anything). Reworked to route through the background script explicitly instead, the same way the native-mirror pattern above already works around other Safari cross-context gaps: `background/index.ts`'s `getKeybindingsSetResponse` calls `broadcastKeybindingsChanged`, which queries open tabs matching `*://*.rateyourmusic.com/charts/*` (the chart-shortcuts content script's own `manifest.ts` pattern) and `browser.tabs.sendMessage`s each one a `KeybindingsUpdatedMessage` (`messaging.ts` — a one-way background→tab broadcast, deliberately outside the `BackgroundRequest`/`BackgroundResponse` request-response union since it has no `id` and isn't a reply to anything); `subscribeToChartShortcutBindings` now listens for that via `browser.runtime.onMessage` instead of `storage.onChanged`. This broadcast runs before the native mirror and independently of whether it succeeds, and works the same way on Chrome/Firefox too (no native host required for the broadcast itself) — follow this same shape (background-mediated tab broadcast, not `storage.onChanged`) for any future setting that needs live cross-context sync in this codebase.

`chart-shortcuts/binding.ts` keys combos off `event.code`, not `event.key`, specifically because `event.key` is shift-dependent (shift+1 reports `"!"` on a US layout) — this is the same reasoning to reuse for any other keyboard-shortcut-recording UI added to this codebase. Tradeoff: `event.code` is physical-position-based, so on a non-QWERTY layout the recorded/displayed key won't match the physical keycap label — the same tradeoff VS Code and browsers make for the same reason.

`binding.ts`'s `isMacPlatform` (real `navigator.platform` detection) exists as a module-level export, but `formatCombo` takes platform as an explicit `mac: boolean = isMacPlatform` parameter rather than reading the module constant directly inside the function. This is deliberate, not stylistic: baking platform detection into a value computed once at module load makes the function's output depend on whatever machine happens to run it, and Node's own `navigator.platform` reports the real host OS (confirmed 2026-08-22: Node 22 on this Mac dev machine reports `"MacIntel"`), so vitest tests for the non-Mac label set started failing purely because the test suite happened to run on a Mac — not because the code was wrong. Any future platform-conditional formatting function in this codebase should take the platform as an explicit parameter with a real-detection default, the same way, so tests can pin it independent of the host machine.

Rendering Mac's native modifier glyphs (`⌃⌥⇧⌘`) needs more than the right Unicode code points: `src/modules/popup/styles.ts`'s shortcut `chip` style originally used a monospace font (`ui-monospace, SFMono-Regular, monospace`), which doesn't carry San Francisco's custom glyph art for those code points and fell back to a flat "^"-style caret, visibly different from Safari's own menu rendering (which uses the system UI font). Switched to the same `-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif` stack already used by the popup's `root` style, which fixed the glyph shapes. Even after that fix, removing the space between glyphs (to match native macOS's tight packing, e.g. `⇧⌘N`) looked "more squished" than genuine native menus per the user's own visual comparison — so some rendering-metrics gap beyond just font-family remains between this extension's rendering context and native macOS menu rendering, not fully diagnosed. Shipped with a space between glyphs (`⌘ ⇧ O`) as the accepted compromise; revisit only if pixel-exact native fidelity is specifically wanted (see `docs/todo.md`).
